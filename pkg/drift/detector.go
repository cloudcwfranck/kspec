...
func (d *Detector) removeVolatileFields(obj map[string]interface{}) {
	...
	// Remove Kyverno default fields that aren't meaningful drift
	if spec, ok := obj["spec"].(map[string]interface{}); ok {
		// Remove 'admission: true' as it's a Kyverno default
		if admission, hasAdmission := spec["admission"].(bool); hasAdmission && admission {
			delete(spec, "admission")
		}
		// webhookTimeoutSeconds has a default value
		if timeout, hasTimeout := spec["webhookTimeoutSeconds"].(float64); hasTimeout && timeout == 10 {
			delete(spec, "webhookTimeoutSeconds")
		}
		// failurePolicy defaults to "Fail"
		if failurePolicy, hasPolicy := spec["failurePolicy"].(string); hasPolicy && failurePolicy == "Fail" {
			delete(spec, "failurePolicy")
		}
	}
}
